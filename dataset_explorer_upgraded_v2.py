# streamlit run dataset_explorer_v11.py
    ...
    try:
        ...
        for element in elements:
            if element.name in ['h2', 'h3']:
                ...
                # Check if this is a valid metadata table (must have 'type' or 'data_type')
...
# =============================================================================
# 5. VISUALIZATION ENGINE (THE HYBRID MAP)
# =============================================================================

@st.cache_data
def create_hybrid_map(df: pd.DataFrame, target_node: str = None) -> go.Figure:
    """
    Generates the Plotly figure for the Universe Map.
    
    Features:
    1. 'Clustered Spring Layout': Groups datasets by Category visually.
    2. 'HUD Logic': Only draws connection lines if a Target is selected.
    3. 'Focus Mode': Dims irrelevant nodes when a Target is selected.
    """
    if df.empty: return go.Figure()
    
    # ---------------------------------------------------------
    # Step 1: Build the NetworkX Graph for Physics Calculation
    # ---------------------------------------------------------
    G = nx.Graph()
    
    # We aggregate to get unique datasets and their metadata
    ...
    # ---------------------------------------------------------
    # Step 2: Calculate Physics (Node Positions)
    # ---------------------------------------------------------
    categories = sorted(list(set(nx.get_node_attributes(G, 'category').values())))
    
    # Assign Colors to Categories
    colors_palette = px.colors.qualitative.Set3 + px.colors.qualitative.Pastel1
    cat_colors = {cat: colors_palette[i % len(colors_palette)] for i, cat in enumerate(categories)}
    
    # Seed Position Logic: Place Categories in a circle to create "Clusters"
    ...
    # ---------------------------------------------------------
    # Step 4: Construct Plotly Traces
    # ---------------------------------------------------------
    ...
    # Separate lists for marker line properties to avoid Plotly ValueError
    ...
    for node in G.nodes():
        ...
        if target_node:
            # Focus Mode Active
            if node == target_node:
                node_color.append('#58A6FF') # Highlight Target (Blue)
                node_size.append(25)
    ...
    node_trace = go.Scatter(
        ...
        marker=dict(
            color=node_color,
            size=node_size,
            # BUG FIX: Pass lists to line properties correctly
            line=dict(width=node_line_width, color=node_line_color)
        )
    )

    # ---------------------------------------------------------
    # Step 5: Final Layout Assembly
    # ---------------------------------------------------------
    fig = go.Figure(
...
def generate_manual_sql(selected_datasets: List[str], df: pd.DataFrame) -> str:
    ...
    # 1. Build a simplified connection graph
    G_full = nx.Graph()
    joins = get_possible_joins(df)
